...
public protocol OpenRouterClientProtocol: Sendable {
    ...
    func complete(
        prompt: String, model: String, systemPrompt: String?, maxTokens: Int?
    ) async throws -> String

    func completeWithVision(
        prompt: String, imageData: [Data], model: String, maxTokens: Int?
    ) async throws -> String
}
...
public actor OpenRouterClient: OpenRouterClientProtocol {
    ...
    public func complete(request: ChatCompletionRequest) async throws -> ChatCompletionResponse {
        ...
        urlRequest.setValue(config.httpReferer, forHTTPHeaderField: "HTTP-Referer")
        urlRequest.setValue(config.appTitle, forHTTPHeaderField: "X-Title")
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            ...
            throw ChoirHelperError.apiError(statusCode: httpResponse.statusCode, message: body)
        }
    ...
    public func complete(
        prompt: String, model: String, systemPrompt: String? = nil, maxTokens: Int? = nil
    ) async throws -> String {
        ...
        let request = ChatCompletionRequest(model: model, messages: messages, maxTokens: maxTokens)
        let response = try await complete(request: request)
        guard let content = response.firstContent else {
            throw ChoirHelperError.apiError(statusCode: 0, message: "Empty response")
        }
        return content
    }

    public func completeWithVision(
        prompt: String, imageData: [Data], model: String, maxTokens: Int? = nil
    ) async throws -> String {
        ...
        let request = ChatCompletionRequest(model: model, messages: [message], maxTokens: maxTokens)
        let response = try await complete(request: request)
        guard let content = response.firstContent else {
            throw ChoirHelperError.apiError(statusCode: 0, message: "Empty response")
        }
