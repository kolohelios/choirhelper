...
struct NoteTests {
    ...
    @Test("NoteType relative durations")
    func noteTypeDurations() {
    ...
    @Test("Note is Codable round-trip")
    func codableRoundTrip() throws {
        let original = Note(
            pitch: Pitch(step: .a, alter: -1, octave: 3),
            duration: 0.5,
            noteType: .eighth,
            lyric: Lyric(text: "test", syllabic: .single),
            dynamic: .mf
        )
