...
struct ScoreTests {
    ...
    static func makeScore() -> Score {
        ...
        return Score(
            ...
            tempo: 120,
            parts: [tenorPart, pianoPart],
            userPartTypes: [.tenor]
        )
    }

    @Test("Score identifies vocal parts")
    func vocalParts() {
    ...
    @Test("Score identifies accompaniment parts")
    func accompanimentParts() {
        let score = ScoreTests.makeScore()
        #expect(score.accompanimentParts.count == 1)
        #expect(score.accompanimentParts.first?.partType == .piano)
    }

    @Test("Score identifies user parts")
    func userParts() {
        let score = ScoreTests.makeScore()
    ...
    @Test("Score measure count from first part")
    func measureCount() {
        let score = ScoreTests.makeScore()
        #expect(score.measureCount == 1)
    }

    @Test("Score calculates duration in seconds")
    ...
    @Test("Score is Codable round-trip")
    ...
    @Test("Empty score has zero duration")
    func emptyScoreZeroDuration() {
        let score = Score(
            title: "Empty",
            keySignature: KeySignature(fifths: 0),
            timeSignature: TimeSignature(beats: 4, beatType: 4),
