...
struct PitchTests {
    ...
    @Test("A4 frequency is 440 Hz")
    func concertAFrequency() {
        let a4 = Pitch(step: .a, octave: 4)
        #expect(abs(a4.frequency - 440.0) < 0.01)
    }

    @Test("Middle C frequency is ~261.63 Hz")
    func middleCFrequency() {
        let middleC = Pitch(step: .c, octave: 4)
        #expect(abs(middleC.frequency - 261.63) < 0.01)
    }

    @Test("Sharp raises MIDI number by 1")
    func sharpRaises() {
    ...
    @Test("Flat lowers MIDI number by 1")
    func flatLowers() {
