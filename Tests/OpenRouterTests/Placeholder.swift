...
final class MockURLProtocol: URLProtocol, @unchecked Sendable {
    nonisolated(unsafe) static var mockHandler:
...
struct OpenRouterClientTests {
    ...
    @Test("Complete sends correct request structure")
    func completeSendsCorrectRequest() async throws {
        ...
        let client = OpenRouterClient(
            config: testConfig, session: mockSession()
        )
        _ = try await client.complete(
            prompt: "test prompt", model: "test/model",
            systemPrompt: nil, maxTokens: nil
        )
    ...
    @Test("Complete returns text content")
    func completeReturnsContent() async throws {
        MockURLProtocol.mockHandler = { _ in
            successResponse(json: sampleResponseJSON)
        }
    ...
    @Test("Complete with system prompt includes system message")
    func completeWithSystemPrompt() async throws {
    ...
    @Test("Vision request includes base64 images")
    func visionRequestIncludesImages() async throws {
        MockURLProtocol.lastRequestBody = nil
        MockURLProtocol.mockHandler = { _ in
            successResponse(json: sampleResponseJSON)
        }
        ...
        _ = try await client.completeWithVision(
            prompt: "analyze this", imageData: [fakeImage],
            model: "test/vision", maxTokens: nil
        )
...
@Suite("ChatCompletionResponse")
struct ChatCompletionResponseTests {
    @Test("Decodes standard response")
...
struct ModelTaskTests {
    @Test("Vision OCR has default model")
    func visionOCRModel() {
        let task = ModelTask.visionOCR
        #expect(task.defaultModel.isEmpty == false)
    }

    @Test("All tasks have different defaults for different purposes")
    func allTasksHaveModels() {
        let tasks: [ModelTask] = [
            .visionOCR, .musicXMLGeneration, .coaching,
        ]
        for task in tasks {
            #expect(task.defaultModel.contains("/"))
        }
    }
}
...
struct MessageContentTests {
    @Test("Text content encodes as string")
    ...
    @Test("Text content decodes from string")
    func textDecodesFromString() throws {
        let data = Data("\"hello\"".utf8)
        let content = try JSONDecoder().decode(
            MessageContent.self, from: data
        )
