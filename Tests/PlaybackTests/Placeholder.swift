...
struct MIDISchedulerTests {
    ...
    @Test("Generates events for non-rest notes only")
    func generatesEventsForNotes() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
        // 3 pitched notes, 1 rest = 3 events
        #expect(schedule.events.count == 3)
    }

    @Test("Events are sorted by start beat")
    func eventsSortedByStartBeat() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
        for i in 1..<schedule.events.count {
            #expect(
                schedule.events[i].startBeat
                    >= schedule.events[i - 1].startBeat
            )
        }
    }

    @Test("Total beats matches sum of durations")
    func totalBeatsCorrect() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
    ...
    @Test("Total seconds calculated from tempo")
    func totalSecondsFromTempo() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
    ...
    @Test("MIDI note numbers are correct")
    func midiNoteNumbers() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
    ...
    @Test("Event start beats are sequential")
    func eventStartBeatsSequential() {
        let schedule = scheduler.schedule(
            score: MIDISchedulerTests.simpleScore()
        )
    ...
    @Test("Multi-part score generates events for all parts")
    func multiPartEvents() {
        let measure = Measure(
            number: 1,
            notes: [
                Note(
        ...
        let score = Score(
            ...
            parts: [
                ...
                Part(
                    name: "Alto", partType: .alto,
                    measures: [measure],
                    midiChannel: 1, midiProgram: 52
                ),
            ]
        )
    ...
    @Test("Dynamic affects velocity")
    func dynamicAffectsVelocity() {
        let measure = Measure(
            number: 1,
            notes: [
                Note(
...
@Suite("MIDIEvent")
struct MIDIEventTests {
    @Test("End beat is start plus duration")
    func endBeat() {
        let event = MIDIEvent(
            partIndex: 0, midiNote: 60, velocity: 80,
            startBeat: 2.0, durationBeats: 1.5,
