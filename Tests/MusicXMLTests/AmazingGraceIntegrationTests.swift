...
@Suite("Amazing Grace Integration")
struct AmazingGraceIntegrationTests {
    let parser = MusicXMLParser()

    func loadAmazingGrace() throws -> Score {
        // Navigate from test bundle to Resources directory
        let testFile = #filePath
        let projectRoot =
            URL(fileURLWithPath: testFile)
            .deletingLastPathComponent()  // MusicXMLTests/
            .deletingLastPathComponent()  // Tests/
            .deletingLastPathComponent()  // project root
        let xmlURL = projectRoot
            .appendingPathComponent("Resources")
            .appendingPathComponent("ExamplePieces")
            .appendingPathComponent("amazing_grace.musicxml")
        let data = try Data(contentsOf: xmlURL)
        return try parser.parse(data: data)
    }

    @Test("Parses Amazing Grace successfully")
    func parsesSuccessfully() throws {
        let score = try loadAmazingGrace()
        #expect(score.title == "Amazing Grace")
    }

    @Test("Has five parts: SATB + Piano")
    func hasFiveParts() throws {
        let score = try loadAmazingGrace()
    ...
    @Test("Tempo is 100 BPM")
    func tempo() throws {
        let score = try loadAmazingGrace()
        #expect(score.tempo == 100)
    }

    @Test("Soprano has 16 measures")
    func sopranoMeasureCount() throws {
        let score = try loadAmazingGrace()
        #expect(score.parts[0].measures.count == 16)
    }

    @Test("All parts have same number of measures")
    func allPartsEqualMeasures() throws {
        let score = try loadAmazingGrace()
    ...
    @Test("Soprano first real note is G4")
    func sopranoFirstNote() throws {
        let score = try loadAmazingGrace()
    ...
    @Test("Soprano has lyrics")
    func sopranoHasLyrics() throws {
        let score = try loadAmazingGrace()
        let soprano = score.parts[0]
        let notesWithLyrics = soprano.measures.flatMap(\.notes)
            .filter { $0.lyric != nil }
        #expect(notesWithLyrics.count > 10)
    }

    @Test("Four vocal parts and one accompaniment")
    func partClassification() throws {
        let score = try loadAmazingGrace()
